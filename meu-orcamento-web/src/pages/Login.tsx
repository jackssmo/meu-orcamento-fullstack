...
export function Login() {
  ...
  async function handleLogin(e: FormEvent) {
    ...
    try {
      ...
      // Se der erro (ex: senha errada), mostramos a mensagem do teu backend
      setError(err.response?.data || 'E-mail ou palavra-passe incorretos.');
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-8">
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            ...
            <div>
              ...
              <div className="flex justify-end mt-1">
                <a href="#" className="text-sm text-indigo-600 hover:underline">Esqueceu a palavra-passe?</a>
              </div>
            </div>
          ...
          <p className="text-center text-gray-500 text-sm mt-6">
            Não tem uma conta? <Link to="/register" className="text-indigo-600 hover:underline font-medium">Registe-se</Link>
          </p>
        </div>
      </div>
    </div>
